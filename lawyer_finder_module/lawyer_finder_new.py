# lawyer_finder.py - Clean version with proper indentation
from sqlalchemy import create_engine
from sqlalchemy.orm import sessionmaker
from geopy.distance import geodesic
from models import Lawyer, Base, CaseAnalysis
import os
import logging
...
class LawyerFinder:
    ...
    def find_lawyers_by_case(self, case_info: dict, max_distance: int = 25, limit: int = 10):
        """Find lawyers based on case info"""
    ...
    def add_lawyer(self, lawyer_data):
        ...
        try:
            ...
            existing = self.session.query(Lawyer).filter_by(email=lawyer_data.get('email')).first()
            if existing:
                logger.info(f"Lawyer with email {lawyer_data.get('email')} already exists")
                return {'success': False, 'error': 'Lawyer already exists'}
    ...
    def add_sample_data(self):
        ...
        self.session.commit()
        logger.info(f"Added {added_count} sample lawyers")